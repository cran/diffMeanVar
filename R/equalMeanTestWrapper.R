# revised on Oct. 7, 2016
#  add 'logisticReg' and 'chisqGeno' option
# created on Dec. 13, 2015
#
# wrapper function to test for equality of mean

# esFlag = "es" (for ExpressionSet object) or "meth" (for methylumi object)
# meanTestFunc - user defined function with at least 2 arguments: value and group
#  possible meanTestFunc provided by 'varianceDiff' package are:
#   'scoreTestMean', 'scoreTestMean.scaled'
#   'scoreTestMeanWilcox', 'scoreTestMeanWilcox.scaled', 'logisticReg', 'chisqGeno'
#
equalMeanTestWrapper=function(
  es,
  grpVar = "group", 
  ...
  verbose = FALSE)
{
  ...
  ttLst=applier(1:nr, function(i) {
    xi=dat[i,]
    ans=try(resi <- meanTestFunc(value=xi,group=y))
